			   ...
			   const UndoLogInfo * info)
{
	...
	for (i = 0; i < *num_logs; ++i)
	{
		if (logs[i].logno == info->logno)
		{
			...
			/* TODO: full! */
			logs[i].size = Max(logs[i].size, info->size);
...
merge_undo_logs(void)
{
	...
	for (i = 0; i < num_logs; ++i)
	{
		...
		meta_data.tablespace = info->tablespace;
		meta_data.persistence = info->persistence;
		meta_data.size = info->size;

		/*
		 * Round the insert and discard offsets up so that they point to the
		 * first useful byte in the next segment.  That means there is no
		 * logical data and no physical files are expected to exist.
		 */
		meta_data.insert = meta_data.discard =
			((meta_data.insert / UndoLogSegmentSize) + 1) *
			UndoLogSegmentSize + SizeOfUndoPageHeaderData;
