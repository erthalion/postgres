...
StartupXLOG(void)
{
	...
	/*
	 * The undo logs are consistent now. A background worker ("undo worker")
	 * will eventually call ProcessExistingUndoRequests() to check all undo
	 * record sets and apply those belonging to aborted (or not finished)
	 * transactions. We cannot do it here because transaction is needed to
	 * access relations.
	 */
