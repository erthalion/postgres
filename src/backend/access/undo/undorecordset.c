...
typedef struct UndoBuffer
{
	...
	bool		needs_init;
	/*
	 * TODO Remove init_page_offset if temporary undo is moved out of the
	 * shared memory.
	 */
		   ...
		   size_t record_size)
{
	 ...
	 *
	 * TODO The assumption that rolled-back transaction does not need the undo
	 * log anymore is probably wrong. The non-flushed local buffers also cause
	 * problems during discarding, so we need to fix the problem in a better
	 * way (storing the temporary undo in the shared memory?) than by adding
	 * the init_page_offset field to UndoBuffer.
	 */
...
GetAllUndoRecordSets(void)
{
	...
	for (i = 0; i < nlogs; i++)
	{
		...
		/*
		 * TODO Change this to Assert() as soon as we stop using UndoLogSlot
		 * to manage temporary undo - see the TODO comment in front of the
		 * UndoLogSlot definition.
		 */
