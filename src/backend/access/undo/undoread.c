...
UndoRSReaderReadOneForward(UndoRSReaderState *r)
{
	...
	/* same with rmid and type */
	{
		resetStringInfo(&r->buf);
		urp_content = undo_reader_read_bytes(r, urp_content, sizeof(r->node.n.rmid));
		r->node.n.rmid = *(uint8 *) r->buf.data;

		resetStringInfo(&r->buf);
		urp_content = undo_reader_read_bytes(r, urp_content, sizeof(r->node.n.type));
		r->node.n.type = *(uint8 *) r->buf.data;
	}
