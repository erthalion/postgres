...
CheckPointUndoLogs(UndoCheckpointContext *ctx)
{
	 ...
	 * from its private cache, it might expect some fields to remain
	 * intact. In any case, processing of the shared free lists above should
	 * stay as a separate pass as long as we use slist. The point is that the
	 * slot must be removed from the free list before it's passed to
